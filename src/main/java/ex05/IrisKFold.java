...
//hold-out : 훈련세트 + 테스트세트 분리해서 검증
//k-fold crossvalidate : k개로 분리하여 훈련세트 + 테스트세트로 구성한 뒤 k번 반복돌면서 평균 값을 검증 결과로 사용
...
public class IrisKFold {
	...
	public double split(int seed) throws Exception {
		...
		//원본데이터를 훈련데이터와 테스트데이터로 분리
		int percent = 80;
		int trainSize = iris.numInstances() * percent / 100; //전체 데이터의 80%를 훈련데이터로
		...
		Instances test = new Instances(iris, trainSize, testSize);
		
		//class Assigner
		train.setClassIndex(train.numAttributes() - 1); //맨 뒤의 필드를 정답 데이터로 지정
...
//		model = new NaiveBayes();
		...
		//평가
		eval.evaluateModel(model, test); //테스트 데이터로 모델 평가
	...
	public double crossValidation(int seed) throws Exception {
		...
		Instances test = iris.trainCV(numfolds, 0);
...
//		model = new NaiveBayes();
	...
	public void aggregateValue(double[] sum) {
		...
		System.out.println("표준편차값 : " + aggr.getStandardDeviation());
