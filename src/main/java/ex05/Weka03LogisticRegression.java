...
public class Weka03LogisticRegression {
	...
	public void generateModel_Evaluate() throws Exception {
		...
		System.out.println("전체 데이터 건 수 : " + (int)eval.numInstances() + "개");
		System.out.println("정 분류 건 수 : " + eval.correct() + "개");
		double percent = (eval.correct() / eval.numInstances() * 100);
		System.out.println("정 분류율 : " + percent);
	...
	public void saveModel(String path) {
		try {
			...
			System.out.println(path + "에 로지스틱 모델 저장 완료");
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
