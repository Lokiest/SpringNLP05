...
public class RegExpTest {
	...
	public void test2(String str) {
		...
		System.out.println(str);
		res = str.replaceAll("\\bis\\b", "will be");
		System.out.println(res);
	}

	public void test3() {
		String str = "Hello Darling, fucking project is coming~, ㅎㅇ";
...
//		String[] tk = str.split("[\\s,]+");
	 ...
	 * Matcher 클래스
	 * 
	 * Matcher 객체는 특정한 문자열이 주어진 패턴과 일치하는가를 알아보는데 이용된다. Matcher 클래스의 입력값으로는
	 * CharSequence라는 새로운 인터페이스가 사용되는데 이를 통해 다양한 형태의 입력 데이터로부터 문자 단위의 매칭 기능을 지원 받을 수
	 * 있다. 기본적으로 제공되는 CharSequence 객체들은 CharBuffer, String, StringBuffer 클래스가 있다.
	 * 
	 * Matcher 객체는 Pattern 객체의 matcher 메소드를 통해 얻어진다. Matcher 객체가 일단 만들어지면 주로 세 가지
	 * 목적으로 사용된다. [1] 주어진 문자열 전체가 특정 패턴과 일치하는 가를 판단(matches). [2] 주어진 문자열이 특정 패턴으로
	 * 시작하는가를 판단(lookingAt). [3] 주어진 문자열에서 특정 패턴을 찾아낸다(find). - 이들 메소드는 성공 시 true를
	 * 실패 시 false 를 반환한다.
	 * 
	 * 또한 특정 문자열을 찾아 새로운 문자열로 교체하는 기능도 제공된다.
	 * 
	 * # appendRepalcement(StringBuffer sb, String replacement) 메소드는 일치하는 패턴이 나타날
	 * 때까지의 모든 문자들을 버퍼(sb)로 옮기고 찾아진 문자열 대신 교체문자열(replacement)로 채워 넣는다.
	 * 
	 * # 또한 appendTail(StringBuffer sb) 메소드는 캐릭터 시퀀스의 현재 위치 이후의 문자들을 버퍼(sb)에 복사해
	 * 넣는다.
	 */
	public void test5() {
